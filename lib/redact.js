 ...
 */
const process = function(rules, request) {
...
const dataReplacer = function(value, pattern, level, fullReplacementText) {
  ...
  if(level === 'partial'){
    ...
    if(fullReplacementText){
      replaceWith = fullReplacementText;
    }else{
      ...
      config.stringTests = [
        {
          ...
          pattern : new RegExp(`${defaultPatterns[rule.type].pattern}`), 
          replacer: (value, pattern) => dataReplacer(value, pattern, rule.level, options?.fullReplacementText)
        }
